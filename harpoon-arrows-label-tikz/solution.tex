% Source - https://tex.stackexchange.com/a/762315
% Posted by cfr
...
\long\protected\def\dualharpoon@aux[#1]#2#3#4#5{%
  \path (#4.center); \pgfgetlastxy{\temp@ax}{\temp@ay}
  \path (#5.center); \pgfgetlastxy{\temp@bx}{\temp@by}
  \pgfmathsetmacro\temp@angle{atan2(\temp@by-\temp@ay,\temp@bx-\temp@ax)}
  \draw[-foo,mystyle] (#4.{\temp@angle - 10}) -- 
    node[
        anchor={90+\temp@angle},
        inner sep=0pt,#1
    ] {#2} 
...
\begin{document}
\begin{tikzpicture}[
    foo/.tip={Stealth[harpoon,swap]},scale=1.5,
